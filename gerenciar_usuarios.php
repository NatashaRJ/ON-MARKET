...
try {
    // Recupera os usuários com base na busca
    $nome = isset($_POST['nome_busca']) ? trim($_POST['nome_busca']) : '';
    if (!empty($nome)) {
        $stmt = $conn->prepare("SELECT id_cad, nome, data_nascimento, sexo, nome_materno, cpf, email, telefone_celular, telefone_fixo, endereco, cep, cidade, login, senha, nivel_acesso, data_cadastro 
                                FROM cadastro 
                                WHERE nome LIKE :nome");
        $stmt->execute([':nome' => "$nome%"]);
    } else {
        // Caso não haja busca, retorna todos os usuários
        $stmt = $conn->query("SELECT id_cad, nome, data_nascimento, sexo, nome_materno, cpf, email, telefone_celular, telefone_fixo, endereco, cep, cidade, login, senha, nivel_acesso, data_cadastro 
                              FROM cadastro");
    }
...
<body>
    ...
    <form method="POST" action="gerenciar_usuarios.php">
        <label for="nome_busca">Buscar usuário por nome:</label>
        <input type="text" name="nome_busca" id="nome_busca" placeholder="Digite o nome" value="<?php echo isset($_POST['nome_busca']) ? htmlspecialchars($_POST['nome_busca']) : ''; ?>" required>
